...
const MyProfile = () => {
  ...
  const debouncedOnChange = debounce(async (file) => {
    try {
      ...
      formData.append("user_image_key", _.get(user, "[0].user_image_key", ""));
  ...
  const handleEdit = () => {
    try {
      form.setFieldsValue({
        alter_mobile_number:
          _.get(user, "[0].alter_mobile_number", "") || "+910000000000",
  ...
  const handleLogout = async () => {
    try {
      ...
      localStorage.removeItem("chgi5kjieaoyaiackaiw_bbcqgy4akacsaiq_bbcqgyyaq");
            ...
            {open === "form" ? (
              ...
              >
                <div className="flex gap-y-4 flex-col py-4">
                  <Dragger showUploadList={false} onChange={uploadtocloud}>
                    <p className="ant-upload-drag-icon">
                      <CloudUploadOutlined className="!text-slate-400" />
                    </p>
                    <p className="ant-upload-text">upload from device</p>
                  ...
                  <Divider>
                    <div className="!text-primary_color">or</div>
                  </Divider>
